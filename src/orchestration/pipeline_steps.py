...
def output_paths(config: PipelineConfig) -> dict[str, str]:
    ...
    return {
        ...
        "operational_risk": f"data/processed/operational_risk_{tag}.csv",
        "operational_risk_report": f"reports/operational_risk_diagnostics_{rtag}.md",
        "operational_risk_json": f"reports/operational_risk_diagnostics_{rtag}.json",
        "operational_state_distribution": f"dashboards/operational_state_distribution_{tag}.csv",
        "operational_attention_distribution": f"dashboards/operational_attention_distribution_{tag}.csv",
        "operational_dominant_driver": f"dashboards/operational_dominant_driver_{tag}.csv",
        "operational_driver_count": f"dashboards/operational_driver_count_{tag}.csv",
        "operational_events": f"data/processed/operational_events_{tag}.csv",
        "staging_hard_report": f"reports/staging_hard_checks_{rtag}.md",
        "staging_hard_json": f"reports/staging_hard_checks_{rtag}.json",
        "market_data_admission_json": f"reports/market_data_admission_{rtag}.json",
    ...
    steps += [
        ...
        PipelineStep(
            ...
            [
                py,
                "-m",
                "src.pipeline.admission_runner",
                "--input-csv",
                paths["features"],
                "--dataset-id",
                f"hourly_features_{config.market_label}_{config.start}_to_{config.end}",
                "--output-json",
                paths["market_data_admission_json"],
                "--expected-start-utc",
                f"{config.start}T00:00:00Z",
                "--expected-end-utc",
                f"{config.end}T23:00:00Z",
                "--as-of-utc",
                f"{config.end}T23:00:00Z",
            ],
            "Gate feature data before downstream analytics.",
        ),
        PipelineStep(
        ...
        PipelineStep(
            ...
            [
                ...
                *base_args(config),
                ...
                "--operational-risk-output",
                paths["operational_risk"],
            ],
            "Translate risk signals into operational decision-support states.",
        ),
        PipelineStep(
            ...
            [
                ...
                *base_args(config),
                ...
                paths["operational_events"],
            ],
            "Aggregate operational-risk hours into contiguous operational events.",
        ),
        PipelineStep(
            ...
            [
                ...
                *base_args(config),
                ...
                "--operational-risk-output",
                paths["operational_risk"],
                "--operational-risk-report-output",
                paths["operational_risk_report"],
                "--operational-risk-json-output",
                paths["operational_risk_json"],
                "--operational-state-distribution-output",
                paths["operational_state_distribution"],
                "--operational-attention-distribution-output",
                paths["operational_attention_distribution"],
                "--operational-dominant-driver-output",
                paths["operational_dominant_driver"],
                "--operational-driver-count-output",
                paths["operational_driver_count"],
            ],
            "Build operational-risk diagnostic evidence and dashboard tables.",
        ),
    ]
